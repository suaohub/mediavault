...
import { readInputFiles } from '../utils'
...
function ThemePicker() {
  ...
  return (
    <div className="theme-wrap" ref={ref}>
      <button className="btn btn-ghost" title="切换主题" onClick={() => setOpen(o => !o)} style={{ padding: '6px 9px' }}>
        <IcoPalette />
      </button>
...
/* ─── Header ─── */
interface Props {
  onImport: (pairs: { file: File; relativePath: string }[]) => void
}
...
export default function Header({ onImport }: Props) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const pairs = readInputFiles(e.target.files)
